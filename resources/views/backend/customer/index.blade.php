              ...
              <div class="row d-flex jusify-content-center g-4 pb-3">
                    <div class="col-md-3 col-sm-3">
                       <div class="eg-card-two success">
                           <h5 class="title">{{ translate('Total Deposit Amount') }}</h5>
                           <h2 class="number">{{  currency_symbol(). $data['total_deposit']}}</h2>
                   ...
                   <div class="col-md-3 col-sm-3">
                       <div class="eg-card-two primary">
                           <h5 class="title">{{ translate('Total Pay Amount') }}</h5>
                           <h2 class="number">{{  currency_symbol().$data['total_pay']}}</h2>
                           <svg width="74" height="78" viewBox="0 0 74 78" fill="none"
                               xmlns="http://www.w3.org/2000/svg">
                               <g opacity="0.3">
                                   <path
                   ...
                   <div class="col-md-3 col-sm-3">
                       <div class="eg-card-two pink">
                           <h5 class="title">{{ translate('Total Pending Amount') }}</h5>
                           <h2 class="number">{{  currency_symbol().$data['total_pending']}}</h2>
                           <svg width="74" height="78" viewBox="0 0 74 78" fill="none"
                               xmlns="http://www.w3.org/2000/svg">
                               <g opacity="0.3">
                                   <path
                ...
                <div class="row mb-35 g-4">
                    ...
                    <div class="col-md-8 text-md-end text-center d-flex justify-content-md-end justify-content-center flex-row align-items-center flex-wrap gap-4">
                        ...
                        <a href="{{route('customer.create')}}" class="eg-btn btn--primary back-btn"><img src="{{asset('backend/images/icons/add-icon.svg')}}" alt="{{translate('Add New')}}"> {{translate('Add New')}}</a>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-3 col-md-6 col-sm-6">
                        <div class="eg-profile-card text-center">
                            <div class="profile-img">
                                @if($customer->image)
                                <img class="rounded-circle" src="{{asset('uploads/users/'.$customer->image)}}" alt="{{$customer->username}}">
                                @else
                                <img class="rounded-circle" src="{{asset('uploads/users/user.png')}}" alt="{{$customer->username}}">
                                @endif
                                @if($customer->country_id)
                                <span class="country-flag">
                                    <img src="{{asset('assets/img/flags/'.$customer->countries?->country_code.'.png')}}" alt="{{$customer->countries?->name}}"> {{$customer->countries?->country_code}}
                                </span>
                                @endif
                                <span id="statusCustomer{{$customer->id}}">
                            ...
                            <div class="profile-bio">
                                <h4>{{$customer->fname .' '. $customer->lname}}</h3>
                                <h6>Customer ID: {{$customer->custom_id}}</h5>
                            </div>
                            <div class="card-action d-flex justify-content-sm-between">
                                <div class="d-flex flex-row justify-content-md-center justify-content-end align-items-center gap-2">
                                    <a href="{{route('customer.edit', $customer->id )}}" title="{{translate('Edit')}}" class="eg-btn add--btn"><i class="bi bi-pencil-square"></i></a>
                                    ...
                                    <a href="{{route('customer.view', ['id'=>$customer->id] )}}" title="{{translate('Profile')}}" class="eg-btn account--btn"><i class="far fa-user"></i></a>
                                     <!-- <a href="{{route('customer.upgrade', $customer->id )}}" title="{{translate('Edit')}}" class="eg-btn add--btn"><i class="bi bi-box-arrow-up"></i></a> -->
                                     <form method="POST" action="{{ route('customer.upgrade', $customer->id) }}">
                                        @csrf
                                         @method('PATCH') 
                                        <input name="id" type="hidden" value="{{$customer->id}}">
                                        <button type="submit" class="eg-btn delete--btn "><i class="bi bi-box-arrow-up"></i></button>
                                    </form>
                               
                               
                                    </div>
                                <div class="form-check form-switch">
                                    <input class="form-check-input customerstatus" type="checkbox" data-activations-status="{{$customer->status}}" data-customer-id="{{$customer->id}}" id="flexSwitchCustomer{{$customer->id}}" {{($customer->status==1)?'checked':''}}>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <h1>{{translate('Customers Not Found')}}!</h1>
                    </div>
                    @endif
                </div>
                ...
                <!-- vip customers -->
                 @php
                    $count=App\Models\User::where('isVip',1)->count();
                 @endphp
                        ...
                        <div class="eg-profile-card text-center">
                            ...
                            <div class="card-action d-flex justify-content-sm-between">
                                <div class="d-flex flex-row justify-content-md-center justify-content-end align-items-center gap-2">
                                    ...
                                    <a href="{{route('customer.view', ['id'=>$customer->id] )}}" title="{{translate('Profile')}}" class="eg-btn account--btn"><i class="far fa-user"></i></a>
                                     <!-- <a href="{{route('customer.upgrade', $customer->id )}}" title="{{translate('Edit')}}" class="eg-btn add--btn"><i class="bi bi-box-arrow-up"></i></a> -->
                                     <form method="POST" action="{{ route('customer.downgrade', $customer->id) }}">
                                        @csrf
                                         @method('PATCH') 
                                        <input name="id" type="hidden" value="{{$customer->id}}">
                                        <button type="submit" class="eg-btn delete--btn "><i class="bi bi-box-arrow-down"></i></button>
                                    </form>
