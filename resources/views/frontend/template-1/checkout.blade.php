...
@section('content')
    ...
    <div class="checkout-section pt-120 pb-120">
        <div class="container">
            <form action="{{ route('customer.payment.method') }}" method="POST" class="require-validation"
                data-cc-on-file="false" data-stripe-publishable-key="{{ get_payment_method('stripe_key') }}" id="payment-form" enctype="multipart/form-data">
                @csrf
    ...
    <div id="form-{{ $payment_method->method_name }}" style="display:none; margin-top: 10px;">
...
<script>
function toggleForm(methodName, isChecked) {
    const customForm = document.getElementById('form-' + methodName);
    const stripeForm = document.getElementById('strip-payment');
    ...
    if (isChecked) {
        ...
        // Hide your custom form if unchecked
        customForm.style.display = 'none';

        // Optionally, show stripe form back if needed
