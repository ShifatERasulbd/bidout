...
class HomeController extends Controller
{
    ...
    public function auction_details(Request $request, $slug)
    {
        ...
        try {
            ...
            return view('frontend.template-' . $templateId . '.live-auction-details', compact('auction_details', 'title', 'meta_description', 'meta_keyward', 'meta_image', 'img_galleries', 'specifications', 'live_auctions', 'lang', 'funFactsDataItem', 'bidHistories', 'randomBlog', 'reviewAllow', 'product_reviews'));
        } catch (\Throwable $th) {
            return view('frontend.errors.index', ['templateId' => $templateId, 'lang' => $lang]);
        }
    }
    ...
    public function blogs(Request $request)
    {
        ...
        try {
            ...
            return view('frontend.template-' . $templateId . '.blog-page', compact('title', 'meta_description', 'meta_keyward', 'meta_image', 'blogs', 'lang'));
        } catch (\Throwable $th) {
            return view('frontend.errors.index', ['templateId' => $templateId, 'lang' => $lang]);
        }
    }
    ...
    public function blog_details(Request $request, $slug)
    {
        ...
        try {
            ...
            return view('frontend.template-' . $templateId . '.blog-details', compact('blog_details', 'title', 'meta_description', 'meta_keyward', 'meta_image', 'lang', 'recentBlogs', 'categories', 'comments', 'randomBlog'));
        } catch (\Throwable $th) {
            return view('frontend.errors.index', ['templateId' => $templateId, 'lang' => $lang]);
        }
    }
                ...
                if( $singleProduct->sale_type==1){
                    if(highest_bid($customer_cart['product_id']) !==0){
                        if($price <= highest_bid($customer_cart['product_id'])  ){
                            return redirect()->back()->with('error', 'Bidding Price Should be gather than Current Bid Price');
                         }
                    }else if( $price <= $singleProduct->min_bid_price ){
                        return redirect()->back()->with('error', 'Bidding Price Should be gather than  Bidding Price');
                    }
    ...
    public function thank_you(Request $request)
    {
        ...
        if (Session::has('orders')) {
            ...
            return view('frontend.template-' . $templateId . '.thankyou', compact('title', 'meta_image', 'meta_description', 'meta_keyward', 'lang', 'orderSingle'));
        } else {
            return view('frontend.errors.index', ['templateId' => $templateId, 'lang' => $lang]);
        }
    }
    ...
    public function search(Request $request)
    {
        ...
        try {
            ...
            return view('frontend.template-' . $templateId . '.search-page', compact('title', 'meta_image', 'meta_description', 'meta_keyward', 'live_auctions', 'lang', 'funFactsDataItem'));
        } catch (\Throwable $th) {
            return view('frontend.errors.index', ['templateId' => $templateId, 'lang' => $lang]);
        }
    }
