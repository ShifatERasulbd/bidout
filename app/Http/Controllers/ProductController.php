...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('features_image')) {
            ...
            if ($features_image != '') {
                ...
                $features_image_path="uploads/products/features/".$features_image_name;
        ...
        if($request->sale_type == 1){
            $products->add_balance_one = $request->add_balance_one;
            $products->add_balance_two = $request->add_balance_two;
            $products->add_balance_three = $request->add_balance_three;
            $products->add_balance_four = $request->add_balance_four;
        }
        $products->slug = $slug;
        $products->save();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($user->id == 1 || $user->id == 2) {
            ...
            return redirect()->route('products.list')->with('error', translate('You have no permission!'));
        }
        $validator = Validator::make($request->all(), [
        ...
        if ($features_image != '') {
            if ($products->features_image != null) {
                if (fileExists('uploads/products/features/', $products->features_image) == true && $products->features_image != null) {
    ...
    public function destroy($id)
    {
        ...
        if ($product_gallery != null) {
            foreach ($product_gallery as $pgallery) {
                if (fileExists('uploads/products/gallery/', $pgallery->image) == true && $pgallery->image != null) {
    ...
    {
        if (Auth::user()->id == 1) {
            return back()->with('error', translate('You have no permission!'));
        }
        $products = Product::findOrFail($id);
        $products->status = 1;
        $products->update();
    ...
    public function gallery_remove()
    {
        ...
        if (Auth::user()->id == 1) {
            $response = ['output' => 'success', 'dataId' => '', 'message' => 'You have no permission!'];
        ...
        if ($dataId) {
            ...
            if ($gallery->delete()) {
                $response = ['output' => 'success', 'dataId' => $dataId, 'message' => $message];
    ...
    public function specification_remove()
    {
        ...
        if (Auth::user()->id == 1 || Auth::user()->id ==2) {
            $response = ['output' => 'success', 'dataId' => $dataId, 'message' => 'You have no permission!'];
    ...
    public function changeStatus()
    {
        ...
        if (Auth::user()->id == 1 || Auth::user()->id ==2) {
            $response = ['output' => 'success', 'statusId' => $status, 'proId' => $productId, 'message' => 'You have no permission!'];
        ...
        if ($products->update()) {
            $response = ['output' => 'success', 'statusId' => $products->status, 'proId' => $products->id, 'message' => $message];
    ...
    public function changeStatusClosed()
    {
        ...
        if (Auth::user()->id == 1) {
            $response = ['output' => 'success', 'statusId' => '', 'proId' => '', 'message' => 'You have no permission!'];
    ...
    public function changeReviewStatus(Request $request)
    {
        ...
        if ($productReview->update()) {
            $response = ['output' => 'success', 'statusId' => $productReview->status, 'revId' => $productReview->id, 'message' => $message];
