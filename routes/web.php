...
Route::group(['middleware' => ['XssSanitization']], function () {
    ...
    Route::post('/changelanguage', array(App\Http\Controllers\LanguageController::class, 'changeLanguage'))->name('language.change');

    // Get State and City
    Route::post('/location/get/state', array(App\Http\Controllers\LocationController::class, 'get_state'))->name('location.get.state');
    Route::post('/location/get/city', array(App\Http\Controllers\LocationController::class, 'get_city'))->name('location.get.city');
    Auth::routes();
    Route::get('/logout', array(App\Http\Controllers\Auth\LoginController::class, 'logout'));
 ...
 * Verification Routes
 */
Route::group(['middleware' => ['auth', 'XssSanitization']], function () {
...
Route::group(['middleware' => ['auth', 'is_verified', 'customer','XssSanitization']],function () {
        Route::controller(App\Http\Controllers\HomeController::class)->group(
            function () {
                ...
                //    Route::get('/thank-you', 'thank_you_bank')->name('thank_you_bank');
            }
        );
        Route::post('/razorpay/success', array(App\Http\Controllers\RazorpayController::class, 'success'))->name('razorpay.success');
        Route::get('/razorpay/error', array(App\Http\Controllers\RazorpayController::class, 'error'))->name('razorpay.error');
    }
);

Route::group( ['prefix'=> 'customer', 'middleware' => ['auth', 'is_verified', 'customer', 'XssSanitization']], function () {
        Route::controller(App\Http\Controllers\HomeController::class)->group(
            function () {
        ...
        Route::get('/paypal/{type}/success', array(App\Http\Controllers\PaypalController::class, 'success'))->name('paypal.success');
        Route::get('/paypal/cancel', array(App\Http\Controllers\PaypalController::class, 'error'))->name('paypal.error');
        Route::get('/stripe/confirm', array(App\Http\Controllers\StripeController::class, 'confirm'))->name('stripe.confirm');
    }
);
    ...
    Route::group(['middleware' => ['XssSanitization']], function () {
        ...
        Route::group(['middleware' => ['admin']],function () {
                ...
                Route::group(['prefix' => 'category'],  function () {
                        Route::controller(App\Http\Controllers\CategoryController::class)->group(
                            function () {
                ...
                Route::group( ['prefix' => 'merchant'],  function () {
                        Route::controller(App\Http\Controllers\MerchantController::class)->group( function () {
                ...
                // Frontend Settings
                Route::group( ['prefix' => 'frontend/settings'],function () {
                ...
                Route::group(['prefix' => 'customer'],  function () {
                        ...
                        Route::controller(App\Http\Controllers\CustomerController::class)->group(
                            function () {
                                ...
                                Route::patch('/{id}/upgrade', 'upgrade')->name('customer.upgrade');
                                Route::patch('/{id}/downgrade', 'downgrade')->name('customer.downgrade');
                ...
                // Deposits
                Route::get('/deposits', array(App\Http\Controllers\DepositController::class, 'index'))->name('deposits.list');
        ...
        Route::group(['prefix' => 'watermark'],function () {
                Route::controller(App\Http\Controllers\WatermarkController::class)->group(
                    function () {
                        Route::get('/', 'index')->name('watermark.list');
                        Route::get('/create', 'create')->name('watermark.create');
                        Route::post('/store', 'store')->name('watermark.store');
                        Route::get('/edit/{id}', 'edit')->name('watermark.edit');
                        Route::post('/update', 'update')->name('watermark.update');
                        // Route::get('/{id}/details', 'show')->name('bidding.details');
                    }
                );
            }
        ...
        Route::group(['prefix' => 'winner'], function () {
                Route::get('/', array(App\Http\Controllers\WinnerController::class, 'index'))->name('winner.list');
            }
        );
    }
);
...
Route::group(['middleware' => ['XssSanitization']], function () {
    Route::get('/{slug}', array(App\Http\Controllers\HomeController::class, 'loadPagesContent'))->name('all_pages');
});
